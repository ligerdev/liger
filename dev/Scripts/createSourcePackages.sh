...
usage:
  ...
  Files and directories are named after liger-<edition>-src-<version>.
  example:
...
VERSION=$1
EDITION=$2
PREFIX=liger-${EDITION}-src-${VERSION}
cd `dirname $0`/..
RESULTDIR=`pwd`
TEMPSOURCES=`mktemp -d -t LigerSourcePackage.XXXXXX`
echo "Temporary directory: ${TEMPSOURCES}"
echo "Creating tar archive..."

echo "  Creating tar sources of repositories..."
git archive --format=tar --prefix=${PREFIX}/ HEAD > ${TEMPSOURCES}/__liger_main.tar || exit 1
cd src/shared/qbs || exit 1
git archive --format=tar --prefix=${PREFIX}/src/shared/qbs/ HEAD > ${TEMPSOURCES}/__liger_qbs.tar || exit 1

echo "  Combining tar sources..."
cd ${TEMPSOURCES} || exit 1
tar xf __liger_main.tar || exit 1
tar xf __liger_qbs.tar || exit 1
