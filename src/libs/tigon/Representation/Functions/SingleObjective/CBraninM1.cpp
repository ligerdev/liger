...
REGISTER_IFUNCTION_FACTORY(CBraninM1)
...
CBraninM1::CBraninM1()
{
    TP_defineNInputs(2);
    TP_defineNOutputs(2);
    TString name("Constrained Branin function M1");
    TString description("A constraint variant of the Branin benchmark problem.\n"
                        "This implementation modifies the traditional Branin "
                        "function such that there are two local optima and one "
                        "global optimum, rather than three global optima of equal "
                        "value. This modification is suggested by the book "
                        "\"Engineering Design via Surrogate Modelling, by Andy "
                        "Keane and others\", where the authors claim that it "
                        "makes the function more representative of engeneering "
                        "functions.\n"
...
CBraninM1::CBraninM1(const CBraninM1& func)
{
...
void CBraninM1::evaluate(const TVector<IElementSPtr> &inputs,
                         const TVector<IElementSPtr> &outputs)
{
    if((inputs.size() == TP_nInputs()) && (outputs.size() == TP_nOutputs())) {
        TVector<double> x = IElementVecToRealVec(inputs);
        TVector<double> y = ConstrainedProblems::CBraninM1(x);
...
void CBraninM1::defineInputPrpts()
{
...
void CBraninM1::defineOutputPrpts()
{
    TStringList          varNames;
    TStringList          varDescriptions;
    TVector<ElementType> typeVec;
