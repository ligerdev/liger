...
**
** This is a modified version of LZ09.cpp from jMetalCpp version 1.0.1
** Filename: LZ09.cpp
...
double LZ09::betaFunction(vector<double> * x, int type) {
    ...
    if (type == 4) {
        ...
        for (int i = 0; i < dim; i++) {
            xx = 2 * x->at(i);
            sum += xx * xx;
            prod *= cos(10.0 * M_PI * xx / sqrt(i + 1.0));
        }
...
void LZ09::objective(vector<double> *x_var, vector <double> *y_obj) {
    ...
    if(nobj_==2)
    {
        if(lm_type==21||lm_type==22||lm_type==23||lm_type==24||lm_type==26)
        {
            double g = 0, h = 0, a, b;
            ...
            for(int n=1;n<nvar_;n++)
            {
                ...
                if(n%2==0){
                    a = psfunc2(x_var->at(n),x_var->at(0),n,lm_type,1);  // linkage
                    aa.push_back(a);
                }
                else
                {
                    b = psfunc2(x_var->at(n),x_var->at(0),n,lm_type,2);
                    bb.push_back(b);
                }

            }

            g = betaFunction(&aa, dm_type);
            h = betaFunction(&bb, dm_type);

            double alpha[2] ;
            alphaFunction(alpha,x_var,2,pm_type);  // shape function
        ...
        if(lm_type==25)
        {
            ...
            for(int n=1;n<nvar_;n++){
                if(n%3==0){
                    a = psfunc2(x_var->at(n),x_var->at(0),n,lm_type,1);
                    aa.push_back(a);
                }
                else if(n%3==1)
                {
                    ...
                    c = psfunc2(x_var->at(n),x_var->at(0),n,lm_type,3);
                    if(n%2==0)    aa.push_back(c);
                    else          bb.push_back(c);
                }
            }
            g = betaFunction(&aa,dm_type);          // distance function
            h = betaFunction(&bb,dm_type);
    ...
    if(nobj_==3)
    {
        if(lm_type==31||lm_type==32)
        {
            ...
            {
                a = psfunc3(x_var->at(n),x_var->at(0),x_var->at(1),n,lm_type);
            ...
            g = betaFunction(&aa,dm_type);
            h = betaFunction(&bb,dm_type);
            e = betaFunction(&cc,dm_type);

            //double * alpha = new double[3];
            double alpha[3] ;
            alphaFunction(alpha,x_var,3,pm_type);  // shape function
            y_obj->at(0) = alpha[0] + h;
            y_obj->at(1) = alpha[1] + g;
