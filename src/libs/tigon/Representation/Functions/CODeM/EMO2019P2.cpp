...
REGISTER_IFUNCTION_FACTORY(EMO2019P2)
...
EMO2019P2::EMO2019P2()
{
    ...
    TP_defineNOutputs(2);
    defineNumDirVars(0);
    TString name("EMO2019P2");
    TString description("EMO2019P2 benchmark problem. A scalable uncertain problem.\n"
               "Composed of a deterministic problem with a concave "
               "Pareto front (WFG4Tunable), and uncertainty in radial direction "
...
EMO2019P2::EMO2019P2(const EMO2019P2& func)
{
...
void EMO2019P2::defineNumDirVars(int k)
{
    if(k<=0) {
        // Default value
...
int EMO2019P2::numDirVars() const
{
    return m_nDirVar;
}

void EMO2019P2::evaluate(const TVector<IElementSPtr> &inputs,
                               const TVector<IElementSPtr> &outputs)
{
    if((inputs.size() == TP_nInputs()) && (outputs.size() == TP_nOutputs()) &&
        ...
        TVector<double> oReal = CODeM::EMO2019P2(iReal, m_nDirVar,
                                                 TP_nOutputs());
...
void EMO2019P2::defineInputPrpts()
{
...
void EMO2019P2::defineOutputPrpts()
{
    TStringList          varNames;
    TStringList          varDescriptions;
    TVector<ElementType> typeVec;
