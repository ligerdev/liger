...
KrigingCascade::KrigingCascade(const TVector<TVector<double>>& XX,
                               const TVector<TVector<double>>& YY, TString basis)
{
    ...
    if(basis == "power") {
        for(int i=0; i<ny; i++) {
            interp = new OrdinaryKriging(new PowerVariogram(XX, yl[i]));
            m_interpolators.push_back(interp);
        }
    } else if(basis=="spherical") {
        for(int i=0; i<ny; i++) {
            interp = new OrdinaryKriging(new SphericalVariogram(XX,yl[i]));
            m_interpolators.push_back(interp);
        }
    } else {
        for(int i=0; i<ny; i++) {
...
KrigingCascade::KrigingCascade(const TVector<KrigingVariogram *>& variograms)
{
    m_produceErrorEstimates = false;
    for(int i=0; i<variograms.size(); i++) {
        m_interpolators.push_back(new OrdinaryKriging(variograms[i]));
    }
...
TVector<OrdinaryKriging*> KrigingCascade::interpolators()
{
    return m_interpolators;
}

void KrigingCascade::append(OrdinaryKriging* interpolator)
{
...
void KrigingCascade::enableErrorEstimates()
{
    ...
    int sz = m_interpolators.size();
    for(int i=0; i<sz; i++) {
        m_interpolators[i]->defineEstimateErrors(true);
    }
}
...
void KrigingCascade::disableErrorEstimates()
{
    ...
    int sz = m_interpolators.size();
    for(int i=0; i<sz; i++) {
        m_interpolators[i]->defineEstimateErrors(false);
    }
}
