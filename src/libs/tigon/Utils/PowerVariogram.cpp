...
PowerVariogram::PowerVariogram(TVector<TVector<double> > x, TVector<double> y)
...
void PowerVariogram::fitVariogram()
{
    ...
    for(int i=0; i<N; i++) {
        for(int j=i+1; j<N; j++) {
            ...
            distZ(k) = std::abs(outputData()[i] - outputData()[j]);
            k++;
        }
    }
...
void PowerVariogram::fitVariogramLS(TVector<TVector<double> > x, TVector<double> y)
{
    if(x.size() != y.size()) {
        return;
    }
