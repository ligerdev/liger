...
void KrigingVariogram::assignVariogramData(TVector<TVector<double> > x,
                                           TVector<double> y)
{
...
void KrigingVariogram::calculateEmpiricalVariogram(TCVectorReal dist, TCVectorReal Zx)
{
    // dist: || xi-xj ||_2
...
void KrigingVariogram::calculateEmpiricalVariogram(TVector<TVector<double> > x,
                                                   TVector<double> y)
{
    if(x.size() != y.size()) {
        return;
...
TCVectorReal KrigingVariogram::interiorPointSolver(TMatrixReal Q, TCVectorReal c,
