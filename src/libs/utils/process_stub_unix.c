    ...
    for (;;) {
        ...
        if (WIFSTOPPED(chldStatus)) {
            ...
            if (isDetached == 2 && isDebug) {
                /* liger was not informed and died while debugging, killing the child */
...
int main(int argc, char *argv[])
{
    ...
    if (argc < ArgEnv) {
        fprintf(stderr, "This is an internal helper of Liger. Do not run it manually.\n");
        return 1;
    }
            ...
            for (;;) {
                ...
                if (nbytes <= 0) {
                    ...
                    if (!isDetached) {
                        isDetached = 2;
                        if (nbytes == 0)
                            fprintf(stderr, "Lost connection to Liger, detaching from it.\n");
                        else
                            perror("Lost connection to Liger, detaching from it");
                    }
                    break;
                } else {
                    int i;
                    for (i = 0; i < nbytes; ++i) {
                        ...
                        default:
                            if (!hadInvalidCommand) {
                                fprintf(stderr, "Ignoring invalid commands from Liger.\n");
                                hadInvalidCommand = 1;
                            }
                        }
                    }
