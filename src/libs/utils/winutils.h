...
LIGER_UTILS_EXPORT QString winErrorMessage(unsigned long error);

// Determine a DLL version
enum WinDLLVersionType { WinDLLFileVersion, WinDLLProductVersion };
LIGER_UTILS_EXPORT QString winGetDLLVersion(WinDLLVersionType t,
                                                const QString &name,
                                                QString *errorMessage);

LIGER_UTILS_EXPORT bool is64BitWindowsSystem();

// Check for a 64bit binary.
LIGER_UTILS_EXPORT bool is64BitWindowsBinary(const QString &binary);
...
//
class LIGER_UTILS_EXPORT WindowsCrashDialogBlocker {
public:
