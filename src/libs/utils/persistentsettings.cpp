...
Context::Context() :
    ligerElement(QLatin1String("liger")),
...
private:
    enum Element { LigerElement, DataElement, VariableElement,
...
bool PersistentSettingsWriter::write(const QVariantMap &data, QWidget *parent) const
{
    ...
    if (!saver.hasError()) {
        ...
        w.writeStartElement(ctx.ligerElement);
