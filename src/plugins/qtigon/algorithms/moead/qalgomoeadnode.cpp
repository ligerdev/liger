...
**
****************************************************************************/
#include <qtigon/algorithms/moead/qalgomoeadnode.h>
    ...
    : m_dialog(new QAlgorithmDialog)
{
	...
	QFile img_file(":/qtigon/images/qalgomoeadnode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qalgomoeadnode.svg");
    } else {
...
void QAlgoMOEADNode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QAlgoMOEADNode::readDataProperties(QXmlStreamReader &xmlReader)
{
    ...
    while(!xmlReader.isEndElement() || !(xmlReader.name() == "Properties")) {
        ...
        if(xmlReader.isStartElement()) {
            if(xmlReader.name() == "TigonOperator"){
                ...
                MOEAD* alg = static_cast<MOEAD*>(thisOp);
...
void QAlgoMOEADNode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QAlgoMOEADNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    MOEAD* thisAlg = static_cast<MOEAD*>(data());
    if(thisAlg) {
