...
QAlgoNSGAIIPSANode::QAlgoNSGAIIPSANode()
    : m_dialog(new QAlgorithmDialog)
{
    ...
    setNumberOfInputs(1);
    setData(new NSGAIIPSA);
	
	QFile img_file(":/qtigon/images/qalgonsgaiipsanode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qalgonsgaiipsanode.svg");
    } else {
...
void QAlgoNSGAIIPSANode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QAlgoNSGAIIPSANode::readDataProperties(QXmlStreamReader &xmlReader)
{
    ...
    while(!xmlReader.isEndElement() || !(xmlReader.name() == "Properties")) {
        ...
        if(xmlReader.isStartElement()) {
            if(xmlReader.name() == "TigonOperator"){
                ...
                NSGAIIPSA* alg = static_cast<NSGAIIPSA*>(thisOp);
...
void QAlgoNSGAIIPSANode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QAlgoNSGAIIPSANode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    NSGAIIPSA* thisAlg = static_cast<NSGAIIPSA*>(data());
    if(thisAlg) {
