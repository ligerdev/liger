    ...
    : m_dialog(new QAlgorithmDialog)
{
    ...
    QFile img_file(":/qtigon/images/qalgomoganode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qalgomoganode.svg");
    } else {
...
void QAlgoMOGANode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QAlgoMOGANode::readDataProperties(QXmlStreamReader &xmlReader)
{
    ...
    while(!xmlReader.isEndElement() || !(xmlReader.name() == "Properties")) {
        ...
        if(xmlReader.isStartElement()) {
            if(xmlReader.name() == "TigonOperator"){
                ...
                MOGA* alg = static_cast<MOGA*>(thisOp);
...
void QAlgoMOGANode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QAlgoMOGANode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    MOGA* thisAlg = static_cast<MOGA*>(data());
    if(thisAlg) {
