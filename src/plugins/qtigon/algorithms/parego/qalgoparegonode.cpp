...
**
****************************************************************************/
#include <qtigon/algorithms/parego/qalgoparegonode.h>
...
QAlgoParEGONode::QAlgoParEGONode()
    : m_dialog(new QAlgorithmDialog)
{
	...
	QFile img_file(":/qtigon/images/qalgoparegonode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qalgoparegonode.svg");
    } else {
...
void QAlgoParEGONode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QAlgoParEGONode::readDataProperties(QXmlStreamReader &xmlReader)
{
    ...
    while(!xmlReader.isEndElement() || !(xmlReader.name() == "Properties")) {
        ...
        if(xmlReader.isStartElement()) {
            if(xmlReader.name() == "TigonOperator"){
                ...
                ParEGO* alg = static_cast<ParEGO*>(thisOp);
...
void QAlgoParEGONode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QAlgoParEGONode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    ParEGO* thisAlg = static_cast<ParEGO*>(data());
    if(thisAlg) {        
