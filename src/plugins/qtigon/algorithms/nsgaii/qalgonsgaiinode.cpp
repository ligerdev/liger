...
QAlgoNSGAIINode::QAlgoNSGAIINode()
    : m_dialog(new QAlgorithmDialog)
{
    ...
    QFile img_file(":/qtigon/images/qalgonsgaiinode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qalgonsgaiinode.svg");
    } else {
...
void QAlgoNSGAIINode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QAlgoNSGAIINode::readDataProperties(QXmlStreamReader &xmlReader)
{
    ...
    while(!xmlReader.isEndElement() || !(xmlReader.name() == "Properties")) {
        ...
        if(xmlReader.isStartElement()) {
            if(xmlReader.name() == "TigonOperator"){
                ...
                NSGAII* alg = static_cast<NSGAII*>(thisOp);
...
void QAlgoNSGAIINode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QAlgoNSGAIINode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    NSGAII* thisAlg = static_cast<NSGAII*>(data());
    if(thisAlg) {
