...
QOpProbGeneratorNodeFactory::QOpProbGeneratorNodeFactory()
{
    ...
    if(img_file.exists()) {
        m_toolBoxButton = dtoolBox->registerOperator(this, QString("ProbGenerator operator node."),
                          QString(":/qtigon/images/qopprobgeneratornode.svg"));
    } else {
        m_toolBoxButton = dtoolBox->registerOperator(this, QString("ProbGenerator operator node."),
...
Designer::IProcessNode* QOpProbGeneratorNodeFactory::createProcessNode()
{
    return new QOpProbGeneratorNode();
}

bool QOpProbGeneratorNodeFactory::isSelected()
{
    return m_toolBoxButton->isChecked();
}

void QOpProbGeneratorNodeFactory::unSelect()
{
    m_toolBoxButton->setChecked(false);
}

void QOpProbGeneratorNodeFactory::processButtonTriggered()
{
