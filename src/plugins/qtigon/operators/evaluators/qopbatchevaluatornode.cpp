...
**
****************************************************************************/
#include <qtigon/operators/evaluators/qopbatchevaluatornode.h>
...
QOpBatchEvaluatorNode::QOpBatchEvaluatorNode()
    : m_dialog(new QOperatorDiag)
{
    ...
    setNumberOfInputs(1);
    setData(new BatchEvaluator);

    QFile img_file(":/qtigon/images/qopbatchevaluatornode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qopbatchevaluatornode.svg");
    } else {
...
void QOpBatchEvaluatorNode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QOpBatchEvaluatorNode::readDataProperties(QXmlStreamReader &xmlReader)
{
...
void QOpBatchEvaluatorNode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QOpBatchEvaluatorNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    BatchEvaluator* thisOp = static_cast<BatchEvaluator*>(data());
    if(thisOp) {
