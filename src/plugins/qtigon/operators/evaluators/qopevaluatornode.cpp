...
QOpEvaluatorNode::QOpEvaluatorNode()
    : m_dialog(new QOperatorDiag)
{
    ...
    setNumberOfInputs(1);
    setData(new Evaluator);

    QFile img_file(":/qtigon/images/qopevaluatornode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qopevaluatornode.svg");
    } else {
...
void QOpEvaluatorNode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QOpEvaluatorNode::readDataProperties(QXmlStreamReader &xmlReader)
{
...
void QOpEvaluatorNode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QOpEvaluatorNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    Evaluator* thisOp = static_cast<Evaluator*>(data());
    if(thisOp) {
