...
QOpValidationNode::QOpValidationNode()
    : m_dialog(new QOperatorDiag)
{
    ...
    setNumberOfInputs(1);
    setData(new Validation);

    QFile img_file(":/qtigon/images/qopvalidationnode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qopvalidationnode.svg");
    } else {
...
void QOpValidationNode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QOpValidationNode::readDataProperties(QXmlStreamReader &xmlReader)
{
...
void QOpValidationNode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QOpValidationNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    Validation* thisOp = static_cast<Validation*>(data());
    if(thisOp) {
