...
QOpRandomInitNode::QOpRandomInitNode()
    : m_dialog(new QOperatorDiag)
{
    ...
    setNumberOfInputs(1);
    setData(new RandomInit);

    QFile img_file(":/qtigon/images/qoprandominitnode.svg");
    if(img_file.exists()) {
        setSvg(":/qtigon/images/qoprandominitnode.svg");
    } else {
...
void QOpRandomInitNode::updateProcessState(ProcessState state)
{
    switch(state) {
...
void QOpRandomInitNode::readDataProperties(QXmlStreamReader &xmlReader)
{
...
void QOpRandomInitNode::writeDataProperties(QXmlStreamWriter &xmlWriter)
{
...
void QOpRandomInitNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event)
    RandomInit* thisOp = static_cast<RandomInit*>(data());
    if(thisOp) {
