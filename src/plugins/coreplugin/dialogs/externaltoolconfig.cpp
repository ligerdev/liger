...
QMimeData *ExternalToolModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    md->setData(QLatin1String("application/liger-externaltool-config"), ba);
    return md;
}
                                     ...
                                     const QModelIndex &parent)
{
    ...
    QByteArray ba = data->data(QLatin1String("application/liger-externaltool-config"));
    if (ba.isEmpty())
        return false;
...
{
    return QStringList() << QLatin1String("application/liger-externaltool-config");
}
