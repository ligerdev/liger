...
void GeneralSettings::fillLanguageBox() const
{
    ...
    const QStringList languageFiles = QDir(creatorTrPath).entryList(QStringList(QLatin1String("liger*.qm")));
...
void GeneralSettings::setLanguage(const QString &locale)
{
    ...
    if (settings->value(QLatin1String("General/OverrideLanguage")).toString() != locale)
    {
        QMessageBox::information(Core::ICore::mainWindow(), tr("Restart required"),
                                 tr("The language change will take effect after a restart of Liger."));
    }
    if (locale.isEmpty())
