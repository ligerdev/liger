    ...
    //m_toggleSideBarButton(new QToolButton)
{
    ...
    setWindowTitle(tr("Liger"));
    if (!Utils::HostOsInfo::isMacHost())
        QApplication::setWindowIcon(QIcon(QLatin1String(Constants::ICON_LIGERLOGO_128)));

    QCoreApplication::setApplicationName(QLatin1String("Liger"));
    ...
    // m_designerManager = new DesignerManager(this);
    ...
    //if (Utils::HostOsInfo::isAnyUnixHost()), signal(SIGINT, handleSigInt);
...
/*!
 * \brief Register all default menu items. Modified to suit Liger.
 */
void MainWindow::registerDefaultActions()
{
    ...
    if (Utils::HostOsInfo::isMacHost())
        tmpaction = new QAction(icon, tr("About &Liger"), this); // it's convention not to add dots to the about menu
    else
        tmpaction = new QAction(icon, tr("About &Liger..."), this);
    cmd = ActionManager::registerAction(tmpaction, Constants::ABOUT_LIGER, globalContext);
    if (Utils::HostOsInfo::isMacHost())
        cmd->action()->setMenuRole(QAction::ApplicationSpecificRole);
    mhelp->addAction(cmd, Constants::G_HELP_ABOUT);
    tmpaction->setEnabled(true);
    connect(tmpaction, SIGNAL(triggered()), this,  SLOT(aboutLiger()));
    ...
    if (!Utils::HostOsInfo::isMacHost()) { // doesn't have the "About" actions in the Help menu
        ...
        cmd = ActionManager::registerAction(tmpaction, "Liger.Help.Sep.About", globalContext);
        mhelp->addAction(cmd, Constants::G_HELP_ABOUT);
    }
}
