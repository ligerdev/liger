...
pc.autoscale = function() {
  ...
  var defaultScales = {
    "date": function(k) {
      var extent = d3.extent(__.dataScale, function(d) {
    ...
    "number": function(k) {
      var extent = d3.extent(__.dataScale, function(d) { return +d[k]; });
...
pc.render.pdefault = function() {
  pc.clear('foreground');
  pc.clear('highlight');

  pc.renderBrushed.pdefault();
...
pc.renderBrushed.pdefault = function() {
  pc.clear('brushed');
...
pc.createAxes = function() {
  ...
  g = pc.svg.selectAll(".dimension")
	...
	//console.debug(d);
	//console.debug(__.dimensions);
	//console.debug(xscale(d));
...
pc.highlight = function(data) {
  ...
  var bdata = __.dataBoxplot;
  
  var bindex;
  for(i=0;i<bdata.length;i++) {
      if(bdata[i][0]["id"]==data[0]["id"]) {
		  bindex=i;
		  break;
	  }
  }
  
  var M = __.dimensions.length;
  var boxplot_data = new Array(M);
  for(i=0;i<M;i++) { 
	  boxplot_data[i] = bdata[bindex][i];
  }
  //console.debug(boxplot_data);

  boxplot = pc.svg.selectAll(".dimension");
  
  // box: thirdQuartile, firstQuartile
  boxplot.append("svg:rect")
  	.attr("id", "boxplot_id")
	.attr("x",-10)
    .attr("y",function(d) {
        var value = boxplot_data[__.dimensionsOrder[d]]["thirdQuartile"];
		return yscale[d](value);
    })
	.attr("width",20)
	.attr("height", function(d,i) { 
        var distance = yscale[d](boxplot_data[__.dimensionsOrder[d]]["firstQuartile"]) -
            yscale[d](boxplot_data[__.dimensionsOrder[d]]["thirdQuartile"]);
		return distance;
		})
    .attr("stroke",'green')
	.attr("stroke-width",2)
  ...
  // top vertical line: whiskersTop
  boxplot.append("svg:line")
    .attr("id", "boxplot_id")
  	.attr("x1", 0)
    .attr("y1", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["thirdQuartile"]);
  		})
  	.attr("x2", 0)
    .attr("y2", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["whiskersTop"]);
  		})
  	.attr("stroke-width", 2)
    .attr("stroke", "green");
  
  // bottom vertical line: whiskersBottom
  boxplot.append("svg:line")
  .attr("id", "boxplot_id")
	.attr("x1", 0)
    .attr("y1", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["firstQuartile"]);
		})
	.attr("x2", 0)
    .attr("y2", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["whiskersBottom"]);
		})
	.attr("stroke-width", 2)
  ...
  // middle horizonal line: secondQuartile
  boxplot.append("svg:line")
    .attr("id", "boxplot_id")
  	.attr("x1", -10)
    .attr("y1", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["secondQuartile"]);
  		})
  	.attr("x2", 10)
    .attr("y2", function(d) {
        return yscale[d](boxplot_data[__.dimensionsOrder[d]]["secondQuartile"]);
  ...
  // top horizonal line: whiskersTop
  boxplot.append("svg:line")
    .attr("id", "boxplot_id")
  	.attr("x1", -10)
    .attr("y1", function(d) {
  ...
  // bottom horizonal line: whiskersBottom
  boxplot.append("svg:line")
    .attr("id", "boxplot_id")
  	.attr("x1", -10)
    .attr("y1", function(d) {
  ...
  var xRange = d3.scale.linear()
  	.range([xscale("f1"),xscale("f1")+30])
  	.domain([0,1]);
  
  var max_f1 = __.data[0]["f1"];
  var min_f1 = __.data[0]["f1"];
  var len_data = __.data.length;
  for(i=1;i<len_data;i++) {
	  var tvalue =  __.data[i]["f1"];
	  if(tvalue > max_f1) max_f1 = tvalue;
	  if(tvalue < min_f1) min_f1 = tvalue;
  }
   
  var yRange = d3.scale.linear()
  	.range([yscale["f1"](min_f1),yscale["f1"](max_f1)])
  	.domain([min_f1,max_f1]);
...
pc.unhighlight = function() {
  ...
  // *********
    ...
    d3.selectAll("#boxplot_id").remove();
